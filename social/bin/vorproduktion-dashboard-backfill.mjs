...
async function reelCoverRendern(datum, slot, beitrag) {
  ...
  const coverBeitrag = structuredClone(beitrag);
  /* Einzelne sehr lange deutsche Komposita passen selbst nach dem
     automatischen Zeilenumbruch nicht in eine feste Cover-Pille. Der
     redaktionelle Reel-Inhalt bleibt unverändert; nur der Cover-Kurztitel
     wird in diesem bekannten Fall gleichbedeutend verdichtet. */
  if (/Gewinnabführungsvertrag/i.test(String(coverBeitrag.kurztitel || ""))) {
    coverBeitrag.kurztitel = "Organschaft: Ist der Vertrag wirksam?";
  }
  const ziel = path.join(
    hosting.dir,
    "vorproduktion",
  ...
  await coverRendern(coverDaten(coverBeitrag, null), ziel, { variante: 0 });
  return true;
}
