...
export function offeneKommentare(medien, eigenerName, ledger, opt = {}) {
  ...
  return offen.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
}
...
Gib für jede id an, ob geantwortet werden soll (antworten), den Grund bei Nein (grund) und den Antworttext (text, null bei Nein).`;
  ...
  return daten.antworten.filter((a) => a.antworten && a.text && a.text.trim().length > 0).map((a) => ({ id: a.id, text: a.text.trim().slice(0, 300) }));
}
...
export async function kommentareBeantworten(ig, ledger, { log = console.log } = {}) {
  ...
  const offen = offeneKommentare(medien, eigener, ledger).slice(0, CONFIG.interaktion.maxAntwortenJeLauf);
  if (!offen.length) return { geprueft: medien.length, beantwortet: 0 };
  log(`Interaktion: ${offen.length} neue Kommentare unter ${medien.length} Beiträgen`);
  ...
  for (const k of offen) {
    ...
    if (!text) { ledger.interaktionen.push({ ...eintrag, uebersprungen: true }); continue; }
    try {
