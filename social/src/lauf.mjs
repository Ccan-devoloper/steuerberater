...
import { themenpool, FAECHER, FEED_KATEGORIEN, feedKategorie, feedFolgeErlaubt } from "./inhalte.mjs";
...
async function main() {
  ...
  if (fremdSumme > 0) {
    log("  Gemeinschaftsbudget: Schwesterkanal pflichtseitig vorbereitet; Pflichtaufrufe dürfen zusätzlich "
      + `Core ${(gemeinschaft.fremdFrei.core || 0).toFixed(4)} · Engagement ${(gemeinschaft.fremdFrei.engagement || 0).toFixed(4)} · `
      + `Research ${(gemeinschaft.fremdFrei.research || 0).toFixed(4)} $ nutzen.`);
  } else {
  ...
  const aufThemenpoolAusweichen = (eintrag) => {
    ...
    const farbtreu = [1, 2, 3].includes(ziel);
  ...
  const textSchreiben = async (eintrag) => {
    ...
    if (eintrag.format === "reel") {
      ...
      if (eintrag.format === "aktuell" || eintrag.format === "loesungsskizze") {
        ...
        try {
          const zielKlausur = [1, 2, 3].includes(Number(eintrag.klausur)) ? Number(eintrag.klausur) : null;
          recherche = eintrag.format === "loesungsskizze"
            ? await loesungsRecherchieren(datum, eintrag.anlass || plan.abendAnlass)
            : await aktuellRecherchieren(datum, bisher, zielKlausur);
          /* Aktuell darf den geplanten Farbslot nicht verlassen. Ein Treffer
             aus der falschen Klausur wird wie ein leeres Rechercheergebnis
             behandelt und durch ein Pool-Thema derselben Farbe ersetzt. */
          const rechercheKlausur = FAECHER[recherche.fach]?.klausur ?? null;
          const falscheFarbe = eintrag.format === "aktuell" && zielKlausur && rechercheKlausur !== zielKlausur;
          const leer = /KEINE_NEUIGKEIT/i.test(recherche.notizen || "") || !recherche.quellen.length || falscheFarbe;
          if (leer) {
            const grund = falscheFarbe
              ? `Recherche landete in ${FEED_KATEGORIEN[rechercheKlausur] || recherche.fach || "unbekannt"} statt ${FEED_KATEGORIEN[zielKlausur]}`
  ...
  /* Letzte Schutzschicht direkt vor Instagram. Sie prüft sowohl die geplante
     Kategorie als auch den tatsächlichen Vorgänger im Ledger. So können alte
     Tagespläne, Recherche-Ausreißer oder Überträge die Farbfolge nicht mehr
     still brechen. */
  const rotationsfolgeErlaubt = (inhalt, eintrag) => {
    if (trocken) return true;
    const nachher = {
      format: eintrag.format,
      fach: inhalt?.fach || eintrag.fach,
      klausur: inhalt?.klausur ?? eintrag.klausur,
      thema: inhalt?.themaId || eintrag.themaId,
    };
    const kategorie = feedKategorie(nachher);
    if (eintrag.klausur != null && kategorie != null && Number(eintrag.klausur) !== kategorie) {
      log(`  ↷ ${eintrag.slot} wartet: Inhalt ist ${FEED_KATEGORIEN[kategorie] || kategorie}, geplant ist ${FEED_KATEGORIEN[eintrag.klausur] || eintrag.klausur}.`);
      return false;
    }
    ...
    log(`  ↷ ${eintrag.slot} wartet: ${FEED_KATEGORIEN[kategorie] || kategorie} würde direkt auf dieselbe Feed-Kategorie folgen.`);
    return false;
  };
  ...
  for (const eintrag of beitraegeFaellig) {
    ...
    try {
      ...
      if (eintrag.format === "reel") {
        ...
        if (echt.bestaetigt) vermerken(ledger, { datum, art: "beitrag", slot: eintrag.slot, zeit: eintrag.zeit, stunde: Math.floor(lokaleMinuten() / 60), format: "reel", thema: reel.themaId, fach: reel.fach, klausur: feedKategorie({ format: eintrag.format, fach: reel.fach, klausur: reel.klausur, thema: reel.themaId }), titel: reel.szenen[0]?.titel || reel.kurztitel, hookTyp: reel.hookTyp, hookMuster: reel.hookMuster, medienId, variante: varianteReel, hashtags: reel.hashtags, stimmeId: r.stimmeId || null, stimmeName: r.stimmeName || null, layout: r.layout || null, dauer: Math.round(r.dauer * 10) / 10, veroeffentlicht: new Date().toISOString() });
      ...
      if (echt.bestaetigt) vermerken(ledger, { datum, art: "beitrag", slot: eintrag.slot, zeit: eintrag.zeit, stunde: Math.floor(lokaleMinuten() / 60), format: eintrag.format, thema: beitrag.themaId, fach: beitrag.fach, klausur: feedKategorie({ format: eintrag.format, fach: beitrag.fach, klausur: beitrag.klausur, thema: beitrag.themaId }), titel: beitrag.folien[0].titel, hookTyp: beitrag.hookTyp, medienId, variante, hashtags: beitrag.hashtags, veroeffentlicht: new Date().toISOString(), karteUrl: karteIndex >= 0 ? urls[karteIndex] : null });
