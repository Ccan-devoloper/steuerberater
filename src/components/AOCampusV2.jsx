...
import aoEinheit2 from "../data/k1-ao-einheit-2.js";
...
const AO_INHALTE = [...aoEinheit1, ...aoEinheit2];
...
const OBER = [
  ...
  { id:"wirksamkeit", label:"Wirksamkeit & Bekanntgabe", module:[306,307,308,312,313], beschreibung:"Nichtigkeit, Bekanntgabe, Bekanntgabewille, Handlungsfähigkeit und Zeitpunkt der Wirksamkeit." },
  { id:"fristen", label:"Fristenberechnung", module:[314], beschreibung:"§ 108 AO, §§ 187/188 BGB und die saubere Trennung von Beginn, Ende und Wochenendregel." },
  { id:"fehler", label:"Fehlerfolgen & Korrekturstrategie", module:[315,316], beschreibung:"Wirksamkeit versus Rechtswidrigkeit, Fehlerfolgen sowie Plan A Einspruch / Plan B Korrektur." },
  { id:"einspruch", label:"Einspruchsverfahren", module:[317,318,319], beschreibung:"Auslegung, Erfolgsformel, Statthaftigkeit, Form, Frist und fristwahrender Eingang." },
];
...
function StandardLoesung({ m, schemaOeffnen }) {
  return <div className="fall__block"><b>Lösung</b><ol>{(m.example?.solution || []).map((s,i)=><li key={i}><AOVerlinkterText text={s} onOpen={schemaOeffnen} compact /></li>)}</ol><AONormkette normen={m.normchain || []} onOpen={schemaOeffnen} /></div>;
}

function FallLoesung({ m, schemaOeffnen }) {
  if (m.id === 320 || m.id === 321) return <AOFallEinheit2Darstellung fallId={m.id} onSchema={schemaOeffnen} />;
  return <StandardLoesung m={m} schemaOeffnen={schemaOeffnen} />;
}
...
function AOSeite({ m, erledigt, umschalten, zurueck, oeffnen, schemaOeffnen }) {
  ...
  const vorher = liste[index-1]; const nachher = liste[index+1];
  ...
  return <article className="lesson ao-lesson">
    ...
    <header className="lesson__kopf"><div><span className="kicker">Klausur 1 · Abgabenordnung · {istFall?"Originalfall":"Lernmodul"} {m.id}</span><h1>{m.title}</h1><div className="tags"><span className="tag tag--fach">{m.difficulty}</span><span className="tag">{m.minutes} Minuten</span><span className="tag">AO Einheit {m.einheit} · PDF-S. {m.sourcePages?.join(", ")}</span></div><p className="ao-lawline">{m.law}</p><AOSchemaVerweise text={m.law} onOpen={schemaOeffnen}/></div><button className="gemeistert" aria-pressed={fertig} onClick={()=>umschalten(m.id)}>{fertig?"✓ bearbeitet":"Als bearbeitet markieren"}</button></header>
    ...
    <Tz nummer={n()} label="Quellen" titel="Fundstellen und Quellenstand"><ul className="liste"><li>AO Einheit {m.einheit} · PDF-S. {m.sourcePages?.join(", ")}</li><li>Tabellen, Schemata und Merkkarten der zugeordneten Seiten werden in ihrer jeweiligen Darstellungsart digital übernommen.</li><li>Maßgebliche Normen laut Einheit: {m.law}</li></ul></Tz>
...
function AOThemenseite({ suche, ober, setOber, erledigt, umschalten, oeffnen, schemaOeffnen }) {
  ...
  return <><div className="pagehead"><div><span className="kicker">Klausur 1 · Abgabenordnung · Lernmodule</span><h1>{suche?`Treffer für „${suche}“`:aktiv.id==="alle"?"AO-Lernmodule nach Oberthemen":aktiv.label}</h1><p className="lead">Die Kurs-Einheit bleibt Quellenmetadatum; gelernt wird nach fachlichen Oberthemen.</p></div><span className="zaehler">{liste.length} Unterthemen</span></div><div className="filter ao-topic-filter">{OBER.map((o)=><button key={o.id} aria-pressed={ober===o.id} onClick={()=>setOber(o.id)}>{o.label}</button>)}</div>{aktiv.id!=="alle"&&<div className="ao-topic-intro"><span className="kicker">Oberthema</span><h2>{aktiv.label}</h2><p>{aktiv.beschreibung}</p></div>}<div className="modules">{liste.map((m)=>{const fertig=erledigt.includes(m.id);return <div key={m.id} className={`modul${fertig?" modul--fertig":""}`} role="button" tabIndex={0} onClick={()=>oeffnen(m.id)} onKeyDown={(e)=>(e.key==="Enter"||e.key===" ")&&(e.preventDefault(),oeffnen(m.id))}><span className="modul__check" role="checkbox" aria-checked={fertig} tabIndex={0} onClick={(e)=>{e.stopPropagation();umschalten(m.id);}}><IconHaken/></span><div><div className="modul__kopf"><span>{oberVon(m).label}</span><span>Unterthema · Modul {m.id}</span><span>Quelle: AO Einheit {m.einheit} · S. {m.sourcePages.join(", ")}</span></div><h3>{m.title}</h3><div className="modul__norm">{m.law}</div><AOSchemaVerweise text={m.law} onOpen={schemaOeffnen} compact stopPropagation/></div><span className="modul__an">öffnen →</span></div>})}</div>{liste.length===0&&<p className="panel">Keine Treffer.</p>}</>;
}

function AOOriginalfaelle({ oeffnen, schemaOeffnen }) {
  return <><div className="pagehead"><div><span className="kicker">Klausur 1 · Abgabenordnung · Originalfälle</span><h1>Originalfälle der AO-Mitschriften</h1><p className="lead">Kursinterne Aufgaben aus den AO-Einheiten; Tabellen- und Rechenfälle werden in ihrer Quellenform dargestellt.</p></div><span className="zaehler">{AO_FAELLE.length} Fälle</span></div><div className="kst-faelle">{AO_FAELLE.map((m)=><article className="panel kst-fallkarte" key={m.id}><div className="panel__head"><div><span className="kicker">Fall {m.id} · AO Einheit {m.einheit} · PDF-S. {m.sourcePages.join(", ")}</span><h2>{m.title}</h2></div><button className="btn btn--klein btn--linie" onClick={()=>oeffnen(m.id)}>Fall öffnen</button></div><p className="kst-fallquelle">{m.law}</p><AOSchemaVerweise text={m.law} onOpen={schemaOeffnen} compact/><div className="kst-sachverhalt"><b>Sachverhalt</b><p>{m.example?.facts}</p></div><details><summary>Lösung anzeigen</summary><div className="fall">{m.id===320||m.id===321?<AOFallEinheit2Darstellung fallId={m.id} onSchema={schemaOeffnen}/>:<StandardLoesung m={m} schemaOeffnen={schemaOeffnen}/>}<div className="fall__block fall__ergebnis"><b>Ergebnis</b><p>{m.example?.result}</p></div></div></details></article>)}</div></>;
}

const QUIZ = [
  ["Was ist die erste Frage der Verwaltungsakte-Falle?",["Welche Korrekturvorschrift gilt?","Liegt überhaupt ein VA vor?","Ist Einspruch begründet?"],1,"Die Quelle startet mit der Existenzfrage nach § 118 AO."],
  ["Welche Fiktion nennt Einheit 2 für einfachen Brief ab 1.1.2025?",["2 Tage","3 Tage","4 Tage"],2,"Die Quelle arbeitet ab 1.1.2025 mit Postaufgabe + 4 Tage."],
  ["Welche zwei Wege zeigt die Fehler-Strategie?",["Einspruch oder Korrekturverfahren","Klage oder Haftung","Vollstreckung oder Außenprüfung"],0,"Plan A ist Einspruch, Plan B Korrekturverfahren."],
  ["Wann ist ein Einspruch nach der Erfolgsformel erfolgreich?",["Nur wenn zulässig","Nur wenn begründet","Wenn zulässig und begründet"],2,"Genau so steht die Erfolgsformel auf PDF-S. 28."],
  ["Welche drei Rechenschritte trennt die Fristenübersicht?",["Beginn, Ende, § 108 Abs. 3 AO","Steuerart, Betrag, Zahlung","Form, Begründung, Klage"],0,"Die Quelle trennt Beginn, Ende und Wochenend-/Feiertagsregel."],
];
function AOTraining(){const[i,setI]=useState(0);const[a,setA]=useState(null);const f=QUIZ[i];return <><div className="pagehead"><div><span className="kicker">Training · AO Einheiten 1–2</span><h1>AO-Schnellcheck</h1><p className="lead">Wiederholung aus den bislang eingearbeiteten {AO_SEITEN} Seiten.</p></div></div><section className="panel kst-quiz"><span className="kicker">Frage {i+1} / {QUIZ.length}</span><h2>{f[0]}</h2><div className="kst-optionen">{f[1].map((x,j)=><button key={x} className={`kst-option${a===null?"":j===f[2]?" richtig":j===a?" falsch":""}`} onClick={()=>a===null&&setA(j)}>{x}</button>)}</div>{a!==null&&<div className="kst-erklaerung"><p>{f[3]}</p><button className="btn btn--klein" onClick={()=>{setA(null);setI((i+1)%QUIZ.length);}}>Nächste Frage</button></div>}</section></>}

function Leer({typ}){return <><div className="pagehead"><div><span className="kicker">Klausur 1 · Abgabenordnung</span><h1>{typ}</h1><p className="lead">Der Bereich ist vorbereitet. Die zugehörigen AO-Unterlagen folgen später und werden mit denselben Querverweisen eingebunden.</p></div></div><section className="panel"><h2>Noch keine Quelldaten</h2><p>Für die AO-Einheiten 1–2 wurden hier bewusst keine fremden Fälle oder Hausaufgaben ergänzt.</p></section></>}

export default function AOCampusV2({ onKlausurwechsel, onFachwechsel }) {
  const [ansicht,setAnsicht]=useState("cockpit");
  const [inhaltId,setInhaltId]=useState(null);
  const [suche,setSuche]=useState("");
  const [ober,setOber]=useState("alle");
  const [schemaZiel,setSchemaZiel]=useState(null);
  const [dunkel,setDunkel]=useState(()=>laden("stb-dunkel",false));
  const fortschritt=useFortschritt("stb-k1-ao-erledigt",AO_IDS);
