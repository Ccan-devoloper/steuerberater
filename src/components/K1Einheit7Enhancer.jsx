...
import aufgaben from "../data/k1-originalfall-aufgaben-einheit7";
import skizzen from "../data/k1-einheit7-skizzen";
...
function skizzeEinsetzen(scope, fallId) {
  ...
  if (vorhanden?.dataset.k1Einheit7 === "true") return;
...
function einheitenTextAktualisieren() {
  ...
  if (beschreibung?.textContent?.includes("USt-Einheiten 1 bis 6")) {
    beschreibung.textContent = beschreibung.textContent.replace("USt-Einheiten 1 bis 6", "USt-Einheiten 1 bis 7");
  }
  const pagehead = document.querySelector("main.page .pagehead");
  const titel = pagehead?.querySelector("h1");
  if (titel?.textContent?.trim() === "USt-Einheiten 1–6") titel.textContent = "USt-Einheiten 1–7";
  const lead = pagehead?.querySelector(".lead");
  if (lead?.textContent?.includes("aus allen sechs Einheiten")) {
    lead.textContent = lead.textContent.replace("aus allen sechs Einheiten", "aus allen sieben Einheiten");
  }
}
...
export default function K1Einheit7Enhancer() {
  useEffect(() => {
    let frame = null;
    const planen = () => {
