...
function findeRail() {
  return document.querySelector(".rail__nav");
}
...
function IconHausaufgabe() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M5 3.5h10.5L19 7v13.5H5z" fill="none" stroke="currentColor" strokeWidth="1.8" />
      <path d="M15.5 3.5V7H19M8 11h8M8 14.5h8M8 18h5" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
    </svg>
  );
}

function HausaufgabenAnsicht({ aktiv, onClose, initialId, onSelect }) {
  ...
  const liste = useMemo(() => {
    ...
    return hausaufgaben.filter((h) => {
      const themaPasst = thema === "alle" || h.themen.includes(thema);
      const text = [h.titel, ...h.themen, ...h.aufgaben, ...h.loesung, ...h.normen].join(" ").toLowerCase();
      return themaPasst && (!q || text.includes(q));
    });
  }, [suche, thema]);

  useEffect(() => {
    if (!aktiv || !initialId) return;
    ...
    <div className="hausaufgaben-view" role="region" aria-label="Hausaufgaben">
      <div className="hausaufgaben-head">
        <div>
          ...
          <h1>Hausaufgaben mit Lösungen</h1>
          <p className="lead">Alle neun Fachtermine als strukturierte, klausurnahe Aufbereitung der bereitgestellten Aufgaben und Musterlösungen.</p>
        </div>
          ...
          return (
            <article className="hausaufgabe" id={h.id} key={h.id}>
              <header className="hausaufgabe__kopf">
                ...
                <span className="hausaufgabe__nummer">{h.termin}</span>
              </header>
              ...
              {module.length > 0 && (
                <div className="hausaufgabe__module">
                  ...
                  <div className="hausaufgabe__links">
                    {module.map((m) => <button key={m.id} onClick={() => modulOeffnen(m)}>Zum Modul {m.id}: {m.title}</button>)}
                  </div>
                </div>
              )}
              ...
              <div className="hausaufgabe__spalten">
                <section><h3>Aufgaben</h3><ul>{h.aufgaben.map((x, i) => <li key={i}>{x}</li>)}</ul></section>
                <section><h3>Lösungsschwerpunkte</h3><ul>{h.loesung.map((x, i) => <li key={i}>{x}</li>)}</ul></section>
              </div>
              <details>
                <summary>Normen & Prüfungshinweise</summary>
                <div className="hausaufgabe__normen">{h.normen.map((n) => <span key={n}>{n}</span>)}</div>
                <p>Prüfungsaufbau: Zurechnung → Zuordnung → Bewertung → Wertansatz → Buchungssatz und Gewinnauswirkung. Handels- und Steuerbilanz bei Abweichungen stets getrennt entwickeln.</p>
              </details>
              <button className="hausaufgabe__anker" onClick={() => onSelect(h.id)}>Direktlink zu dieser Hausaufgabe</button>
            </article>
          );
        })}
      </div>
...
function ModulHinweis({ target, onOpen }) {
  ...
  const match = text.match(/Modul\s+([^\s]+)/);
  ...
  return createPortal(
    <section className="hausaufgaben-modulbox">
      ...
      <h2>Mit diesen Fällen üben</h2>
      <div>{passend.map((h) => <button key={h.id} onClick={() => onOpen(h.id)}>Fachtermin {h.termin}: {h.titel} →</button>)}</div>
    </section>,
    target,
  );
}
...
export default function HausaufgabenPortal() {
  ...
  useEffect(() => {
    const aktualisieren = () => {
      ...
      setLessonTarget(lesson?.querySelector(".blaettern") || null);
    };
    aktualisieren();
  ...
  return (
    <>
      {rail && createPortal(
        <button className="rail__link hausaufgaben-nav" aria-current={aktiv ? "true" : undefined} onClick={() => open()}>
          <IconHausaufgabe />
          Hausaufgaben
        </button>,
        rail,
      )}
      {page && createPortal(<HausaufgabenAnsicht aktiv={aktiv} onClose={() => setAktiv(false)} initialId={auswahl} onSelect={setAuswahl} />, page)}
